...
import { Video, VideoOff, Mic, MicOff, PhoneOff, Maximize2, Minimize2, RotateCcw, Volume2, VolumeX, Signal, SignalZero } from 'lucide-react';
...
function Av({ name, lg }: { name: string; lg?: boolean }) {
  ...
  return <div className={cn('rounded-full bg-gradient-to-br from-slate-600 to-slate-800 border-2 border-white/10 flex items-center justify-center font-black text-white select-none flex-shrink-0', lg?'h-24 w-24 text-3xl':'h-10 w-10 text-sm')}>{i}</div>;
}
...
export function VideoCall({ onClose, targetUserId, targetUserName, alertType, incomingRoomId, incomingCallerNameProp }: VideoCallProps) {
  ...
  // Duration
  useEffect(()=>{
    if(cs==='connected'){ durT.current=setInterval(()=>setDur(d=>d+1),1000); }
    else { if(durT.current){clearInterval(durT.current);durT.current=null;} }
  ...
  const resetCtrl = useCallback(()=>{
    ...
    if(csRef.current==='connected') ctrlT.current=setTimeout(()=>setCtrl(false),4000);
  },[]);
  useEffect(()=>{ resetCtrl(); },[cs,resetCtrl]);
  ...
  useEffect(()=>{
    ...
    qualT.current=setInterval(async()=>{
      ...
      try{
        ...
        let rtt=0;
        stats.forEach((r:any)=>{ if(r.type==='candidate-pair'&&r.state==='succeeded')rtt=r.currentRoundTripTime??0; });
  ...
  // Speaker
  useEffect(()=>{ if(remRef.current)remRef.current.muted=!spk; },[spk]);
    ...
    try{
      ...
      const play=(t:number)=>{
        ...
        [440,480].forEach(f=>{
          ...
          g.gain.setValueAtTime(0.3,t+1.9);
  ...
  const makePC = useCallback((roomId:string,init:boolean):RTCPeerConnection|null=>{
    ...
    pcRef.current=p;
    locStream.current?.getTracks().forEach(t=>p.addTrack(t,locStream.current!));
    p.ontrack=(ev)=>{
      ev.streams[0].getTracks().forEach(t=>{
        if(!remStream.current.getTracks().find(x=>x.id===t.id))remStream.current.addTrack(t);
      });
      if(remRef.current){
        remRef.current.srcObject=remStream.current;
        remRef.current.muted=false;
        remRef.current.play().catch(()=>{});
      }
      setRemVid(true); setCs('connected');
    };
    p.onicecandidate=(ev)=>{
    ...
    p.oniceconnectionstatechange=()=>{ if(p.iceConnectionState==='failed')p.restartIce(); };
    return p;
  },[rtdb,cleanup]);
  ...
  const broadcastCall = useCallback(async(channel:string)=>{
    ...
    onValue(cRef,snap=>{snap.forEach(c=>{void p.addIceCandidate(new RTCIceCandidate(c.val())).catch(()=>{});});});
  ...
  const answerCall = useCallback(async()=>{
    ...
    stopRing(); setErr(null); setCs('connecting');
    ...
    if(roomId!.startsWith('broadcast_')){
      const c=roomId!.split('_')[1];
      if(c)await remove(ref(rtdb,`agency_calls/${c}`)).catch(()=>{});
    }
    const cRef=ref(rtdb,`calls/${roomId}/callerCandidates`);
    onValue(cRef,snap=>{snap.forEach(c=>{void p.addIceCandidate(new RTCIceCandidate(c.val())).catch(()=>{});});});
    candUnsub.current=()=>off(cRef);
  },[rtdb,profile,incomingRoomId,getMedia,makePC,stopRing]);
  ...
  const declineCall = useCallback(async()=>{
    ...
    setCs('ended');
  },[rtdb,profile?.uid,stopRing]);

  // Listen for incoming direct signals
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    if(targetUserId&&targetUserName){
      (async()=>{
        ...
        const offer=await p.createOffer({offerToReceiveAudio:true,offerToReceiveVideo:true});
        await p.setLocalDescription(offer);
        await set(ref(rtdb,`calls/${roomId}`),{offer:{type:offer.type,sdp:offer.sdp},callerId:profile.uid,callerName:profile.name,calleeId:targetUserId,calleeName:targetUserName,createdAt:Date.now()});
        await set(ref(rtdb,`call_signals/${targetUserId}`),{roomId,callerId:profile.uid,callerName:profile.name});
        const aRef=ref(rtdb,`calls/${roomId}/answer`);
        onValue(aRef,async snap=>{if(snap.exists()&&p.currentRemoteDescription===null)await p.setRemoteDescription(new RTCSessionDescription(snap.val())).catch(()=>{});});
        ansUnsub.current=()=>off(aRef);
        const cRef=ref(rtdb,`calls/${roomId}/calleeCandidates`);
  ...
  const toggleMic=()=>{ locStream.current?.getAudioTracks().forEach(t=>{t.enabled=!t.enabled;}); setMic(v=>!v); };
  const toggleCam=()=>{ locStream.current?.getVideoTracks().forEach(t=>{t.enabled=!t.enabled;}); setCam(v=>!v); };
    ...
    <>
      ...
      {mini && cs==='connected' && (
        <div className="fixed bottom-24 right-4 z-[200] flex flex-col items-end gap-2">
          <div className="relative w-36 h-24 rounded-2xl overflow-hidden bg-[#0d1526] border-2 border-white/20 shadow-2xl cursor-pointer flex items-center justify-center" onClick={()=>setMini(false)}>
            <Av name={dn} />
          ...
          <button onClick={()=>setMini(false)} className="h-8 px-3 rounded-xl bg-slate-800 border border-white/10 text-xs text-white font-bold hover:bg-slate-700">Expand</button>
        </div>
      )}
          ...
          <div className="w-full max-w-sm bg-[#0d1526] rounded-3xl border border-white/10 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="flex items-center justify-around px-10 py-8 bg-black/20">
              <div className="flex flex-col items-center gap-2">
                <button onClick={declineCall} className="h-16 w-16 rounded-full bg-red-600 hover:bg-red-500 active:scale-95 flex items-center justify-center shadow-lg transition-all">
                  <PhoneOff className="h-7 w-7 text-white"/>
                </button>
                <span className="text-xs text-slate-400 font-semibold">Decline</span>
              </div>
              <div className="flex flex-col items-center gap-2">
                <button onClick={answerCall} className="h-16 w-16 rounded-full bg-green-500 hover:bg-green-400 active:scale-95 flex items-center justify-center shadow-lg transition-all animate-pulse">
        ...
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="w-full max-w-sm bg-[#0d1526] rounded-3xl border border-white/10 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="flex justify-center pb-8">
              <div className="flex flex-col items-center gap-2">
                <button onClick={hangUp} className="h-16 w-16 rounded-full bg-red-600 hover:bg-red-500 active:scale-95 flex items-center justify-center shadow-lg transition-all">
                  <PhoneOff className="h-7 w-7 text-white"/>
                </button>
      ...
      {!mini && cs==='connected' && (
        <div className="fixed inset-0 z-[200] bg-black">
          <div ref={conRef} className="relative w-full h-full bg-[#0a0d14] overflow-hidden" onMouseMove={resetCtrl} onTouchStart={resetCtrl}>
            ...
            <div className={cn('absolute top-0 inset-x-0 flex items-center justify-between px-4 pt-4 pb-3 transition-all duration-300',ctrl?'opacity-100 translate-y-0':'opacity-0 -translate-y-2 pointer-events-none')}>
              ...
              <div className="flex items-center gap-2">
                <button onClick={()=>setMini(true)} className="h-9 w-9 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur-sm flex items-center justify-center text-white/80 hover:text-white transition-colors"><Minimize2 className="h-4 w-4"/></button>
                <button onClick={toggleFs} className="h-9 w-9 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur-sm flex items-center justify-center text-white/80 hover:text-white transition-colors"><Maximize2 className="h-4 w-4"/></button>
              </div>
            </div>
            {/* Local PiP */}
            <div className={cn('absolute top-20 right-4 rounded-2xl overflow-hidden border-2 border-white/20 shadow-2xl bg-slate-900 w-28 h-20 sm:w-36 sm:h-24 transition-opacity duration-300',ctrl?'opacity-100':'opacity-70')}>
              <video ref={locRef} autoPlay playsInline muted className={cn('w-full h-full object-cover',!cam&&'hidden')}/>
            ...
            <div className={cn('absolute bottom-0 inset-x-0 pb-8 pt-4 px-4 transition-all duration-300',ctrl?'opacity-100 translate-y-0':'opacity-0 translate-y-4 pointer-events-none')}>
              ...
              <div className="flex items-center justify-center gap-3 sm:gap-4">
                <div className="flex flex-col items-center gap-1.5">
                  <button onClick={toggleMic} className={cn('h-14 w-14 rounded-full flex items-center justify-center transition-all active:scale-95 shadow-lg',mic?'bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white':'bg-red-600 hover:bg-red-500 text-white')}>
                    {mic?<Mic className="h-6 w-6"/>:<MicOff className="h-6 w-6"/>}
                  </button>
                  <span className="text-[10px] text-white/60 font-semibold">{mic?'Mute':'Unmuted'}</span>
                </div>
                <div className="flex flex-col items-center gap-1.5">
                  <button onClick={toggleCam} className={cn('h-14 w-14 rounded-full flex items-center justify-center transition-all active:scale-95 shadow-lg',cam?'bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white':'bg-red-600 hover:bg-red-500 text-white')}>
                    {cam?<Video className="h-6 w-6"/>:<VideoOff className="h-6 w-6"/>}
                  </button>
                  <span className="text-[10px] text-white/60 font-semibold">{cam?'Camera':'No cam'}</span>
                </div>
                <div className="flex flex-col items-center gap-1.5">
                  <button onClick={hangUp} className="h-16 w-16 rounded-full bg-red-600 hover:bg-red-500 active:scale-95 flex items-center justify-center shadow-xl shadow-red-900/60 transition-all">
                    <PhoneOff className="h-7 w-7 text-white"/>
                  </button>
                  <span className="text-[10px] text-white/60 font-semibold">End</span>
                </div>
                <div className="flex flex-col items-center gap-1.5">
                  <button onClick={()=>setSpk(v=>!v)} className={cn('h-14 w-14 rounded-full flex items-center justify-center transition-all active:scale-95 shadow-lg',spk?'bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white':'bg-slate-700 hover:bg-slate-600 text-slate-400')}>
                    {spk?<Volume2 className="h-6 w-6"/>:<VolumeX className="h-6 w-6"/>}
                  </button>
                  <span className="text-[10px] text-white/60 font-semibold">{spk?'Speaker':'Muted'}</span>
                </div>
                <div className="flex flex-col items-center gap-1.5">
                  <button onClick={flipCam} className="h-14 w-14 rounded-full bg-white/20 hover:bg-white/30 backdrop-blur-sm flex items-center justify-center text-white transition-all active:scale-95 shadow-lg">
